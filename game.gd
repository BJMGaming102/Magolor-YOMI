...
func resolve_port_priority(id = false):
	...
	return ([p1, p2] if priority == 1 else [p2, p1]) if not id else id
...
func forward_movement(p1_state, p2_state):
	if (p1_state.beats_backdash and not p2_state.beats_backdash):
		return 1
...
func resolve_collisions(p1, p2, step = 0):
	...
	if p1.is_colliding_with_opponent() and p2.is_colliding_with_opponent() and p1.collision_box.overlaps(p2.collision_box):
		...
		if p1.reverse_state:
			push_p1_left = not push_p1_left
		var push_p2_left = (p1.get_facing_int() == - 1)
		if p2.reverse_state:
			push_p2_left = not push_p2_left
		if push_p1_left:
	...
	if step < 5:
		if not p1.clipping_wall and x_pos - p1.collision_box.width < - stage_width:
	...
	if not ReplayManager.playback:
		if not is_waiting_on_player():
			...
			if p1.state_interruptable and not p1_turn:
				...
				someones_turn = true
				p1.busy_interrupt = ( not p1.state_interruptable and not (p1.current_state().interruptible_on_opponent_turn or p1.feinting or negative_on_hit(p1)))
				if not p1.busy_interrupt:
...
func ghost_tick():
	...
	for i in range(simulate_frames):
		...
		if p2.ghost_blocked_melee_attack > 0 and not p2.block_frame_label.visible:
			p2.block_frame_label.show()
			p2.block_frame_label.text = "Parry %s @ %sf" % [p2.ghost_wrong_block, p2.ghost_blocked_melee_attack]
	
		var p1_tick = ghost_simulated_ticks + (p1.hitlag_ticks if not ghost_p2_actionable else 0)
		...
		if p2.ghost_got_hit and not p2.hit_frame_label.visible:
...
func _draw():
	if is_ghost:
		return 
	if not snapping_camera and mouse_pressed:
		draw_circle(camera.position, 3, Color.white * 0.5)
	if draw_stage:
		var line_color = Color.white
		var ceiling_draw_height = - 100000 if not has_ceiling else - ceiling_height
